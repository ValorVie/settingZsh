...
本專案的 editor feature 目前按平台分流：

- macOS / Linux：安裝 **Vim** 和 **Neovim (LazyVim)**
- Windows：安裝 **Neovim (LazyVim)**，不做 `.vimrc` merge，也不額外部署 Vim

兩個編輯器各自運作；在支援 Vim 的平台上，Vim 與 Neovim 也互不影響。

## Editor Shell 整合

Linux / macOS 下，editor 相關 shell 設定不再直接 merge 到整份 `~/.zshrc`。目前結構如下：

- `~/.zshrc`
  - 只保留 `settingZsh bootstrap` 區塊
...
> 註：Windows 目前只部署 Neovim；Vim 這一列主要對應 macOS / Linux。
