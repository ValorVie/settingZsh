...
`setup_mac.sh` / `setup_linux.sh` 在安裝 editor 工具後會自動執行 `reconcile`；若你是從舊版 marker 結構升級，請先跑 `migrate`。
...
先確認 bootstrap 與 editor fragment 是否仍在：
...
如果是從舊版 `.zshrc` markers 升級，先執行一次 `migrate` 再重開 shell。
